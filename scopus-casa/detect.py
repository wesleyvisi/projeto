...
print("carregando... ")
imagens = Imagens("rtsp://10.42.0.96:554/user=admin&password=raspcam&channel=1&stream=0.sdp?",90)
...
while 1:
    ...
    for nitem in range(0, len(lista)):
        ...
        if((item.x >= (item.areaAnterior[0] - 2500)) &
            (item.x <= (item.areaAnterior[0] + 2500)) &
             (item.y >= (item.areaAnterior[1] - 2500)) &
              (item.y <= (item.areaAnterior[1] + 2500))  &
                 (item.w >= (item.areaAnterior[2] - 2500)) &
                  (item.w <= (item.areaAnterior[2] + 2500)) &
                   (item.h >= (item.areaAnterior[3] - 2500)) &
                    (item.h <= (item.areaAnterior[3] + 2500)) ):
            if((item.tempoParado() > 15) & (item.pessoa())):
                cv2.putText(imagens.frame, "{}".format("SOS"), (x, y+250), cv2.FONT_HERSHEY_SIMPLEX, 3, color, 3)
               
            if((item.tempoParado() > 5) & (item.pessoa() == False)):
